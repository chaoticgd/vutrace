...
bool walk_until_pc_equal(AppState &app, u32 target_pc,
                         int step); // Add step to the current snapshot index until pc == target_pc, otherwise do nothing.
void walk_until_mem_access(AppState &app,
                           u32 address); // Add 1 to the current snapshot index until a snapshot reads from/writes to address, otherwise do nothing.
...
void update_gui(AppState &app) {
    if (ImGui::Begin("Snapshots")) snapshots_window(app);
    ImGui::End();
    if (ImGui::Begin("Registers")) registers_window(app);
    ImGui::End();
    if (ImGui::Begin("Memory")) memory_window(app);
    ImGui::End();
    if (ImGui::Begin("Disassembly")) disassembly_window(app);
    ImGui::End();
    if (ImGui::Begin("GS Packet")) gs_packet_window(app);
    ImGui::End();
}
...
void snapshots_window(AppState &app) {
    ...
    if (ImGui::BeginTabBar("tabs")) {
        ...
        if (ImGui::BeginTabItem("Highlighted")) {
            filter = [&](Snapshot &snapshot) {
                std::string disassembly = disassemble(&snapshot.program[snapshot.registers.VI[TPC].SL],
                                                      snapshot.registers.VI[TPC].SL);
                bool is_highlighted =
...
void disassembly_window(AppState &app) {
    ...
    for (std::size_t i = 0; i < VU1_PROGSIZE; i += INSN_PAIR_SIZE) {
        ...
        if (instruction.branch_from_times.size() > 0) {
            ...
            for (auto addrtimes = instruction.branch_from_times.begin();
                 addrtimes != instruction.branch_from_times.end(); addrtimes++) {
        ...
        if (instruction.branch_to_times.size() > 0) {
            ...
            for (auto addrtimes = instruction.branch_to_times.begin();
                 addrtimes != instruction.branch_to_times.end(); addrtimes++) {
                addresses << std::hex << addrtimes->first << " (" << std::dec << addrtimes->second << ") ";
                fallthrough_times -= addrtimes->second;
            }
            ImGui::Text("  -> %s/ ft (%ld)", addresses.str().c_str(), fallthrough_times);
    ...
    while (fread(&packet_type, 1, 1, trace) == 1) {
        switch (packet_type) {
            case VUTRACE_PUSHSNAPSHOT: {
                if (current.registers.VI[TPC].UL >= VU1_PROGSIZE ||
                    current.registers.VI[TPC].UL % INSN_PAIR_SIZE != 0) {
...
void init_gui(GLFWwindow **window) {
    ...
    *window = glfwCreateWindow(1280, 720, "vutrace", NULL, NULL);
    ...
    io.Fonts->AddFontFromMemoryCompressedTTF(&ProggyVectorRegular_compressed_data, ProggyVectorRegular_compressed_size,
                                             font_size);
...
void update_font() {
    ...
    if (use_default_font) {
        ...
        io.FontDefault = io.Fonts->AddFontFromMemoryCompressedTTF(&ProggyVectorRegular_compressed_data,
                                                                  ProggyVectorRegular_compressed_size, font_size);
    }
    ...
    if (ImGui::BeginMainMenuBar()) {
        ...
        if (ImGui::BeginMenu("System")) {
            ...
            ImGui::SetItemTooltip(
                    "Limits the application's refresh rate to decrease impact on CPU. Assuming a 60Hz monitor, the default value (1) is enough.\n0 is unlimited, 60Hz / 2 = 30fps, 60Hz / 3 = 20fps, etc.");
